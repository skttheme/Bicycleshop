 ...
 * @return string   URL
 */
function bicycleshop_current_url() {
	global $wp;
 ...
 * @return bool|string
 */
function bicycleshop_has_active_sidebar() {
	global $bicycleshop_a13;
	$test              = '';
	$page_type         = bicycleshop_what_page_type_is_it();
	$wc_active = bicycleshop_is_woocommerce_activated();
	$shop_with_sidebar = $wc_active && bicycleshop_is_woocommerce_sidebar_page();


	if ( $shop_with_sidebar ) {
		$test = 'shop-widget-area';
	} elseif ( $wc_active && bicycleshop_is_woocommerce() ) {
		return false;
		...
		$meta_id        = get_the_ID();
		$custom_sidebar = $bicycleshop_a13->bicycleshop_get_meta( '_sidebar_to_show', $meta_id );
 ...
 * @return array
 */
function bicycleshop_what_page_type_is_it() {
	global $bicycleshop_a13;
	static $types;
	...
	if ( empty( $types ) ) {
		$types = array(
			...
			'shop'        => bicycleshop_is_woocommerce_activated() && bicycleshop_is_woocommerce(),
			'product'     => bicycleshop_is_woocommerce_activated() && bicycleshop_is_woocommerce() && is_product(),
		);
		...
		$types['page'] = $types['page'] || ($types['404'] && ($bicycleshop_a13->get_option( 'page_404_template_type' ) === 'custom')) || defined('BICYCLESHOP_CUSTOM_PASSWORD_PROTECTED');
	}
 ...
 * @return bool
 */
function bicycleshop_is_no_property_page() {
	global $post;
...
add_filter( 'body_class', 'bicycleshop_body_classes' );
/**
 ...
 * @return array
 */
function bicycleshop_body_classes( $classes ) {
	global $bicycleshop_a13;
	...
	if ( is_active_sidebar( 'side-widget-area' ) ) {
		$side   = $bicycleshop_a13->get_option( 'hidden_sidebar_side' );
		$effect = (int) $bicycleshop_a13->get_option( 'hidden_sidebar_effect' );
		if ( $side === 'right' ) {
	...
	//widgets top margin
	if($bicycleshop_a13->get_option( 'widgets_top_margin' ) === 'off'){
	...
	if($header_type === 'vertical'){
		if(is_rtl()){
			$classes[] = 'header-side-'.$bicycleshop_a13->get_option( 'header_side_rtl' );
		}
		else{
			$classes[] = 'header-side-'.$bicycleshop_a13->get_option( 'header_side' );
		}
	}

	//site layout
	$layout_type = $bicycleshop_a13->get_option( 'layout_type' );
	$classes[] = 'site-layout-'.$layout_type;
	if($layout_type === 'bordered'){
		$borders = array( 'top', 'left', 'bottom', 'right' );
		$borders_on = $bicycleshop_a13->get_option( 'theme_borders' );
	...
	if ( $page_type['blog_type'] && ! defined( 'BICYCLESHOP_NO_RESULTS' ) ) {
		$classes[] = 'posts-list';
	}

	//cart and others not sidebar/title pages of woocommerce
	if ( $page_type['shop'] && ! bicycleshop_is_woocommerce_sidebar_page() ) {
	...
	//password protected
	if ( defined( 'BICYCLESHOP_PASSWORD_PROTECTED' ) ) {
		$classes[] = 'password-protected';
	}

	//custom password page
	if( defined( 'BICYCLESHOP_CUSTOM_PASSWORD_PROTECTED' ) ){
	...
	if( $page_type['404'] ){
		//custom 404 page
		if( $bicycleshop_a13->get_option( 'page_404_template_type' ) === 'custom' ){
			$classes[] = 'custom404';
			$classes[] = 'page';
		}
		else{
 ...
 */
function bicycleshop_get_mid_classes() {
	global $bicycleshop_a13;
	...
	elseif($page_type['shop'] && !bicycleshop_is_woocommerce_sidebar_page()){
		$layout = $bicycleshop_a13->get_option( 'shop_no_major_pages_content_layout' );
	}
	//wish list
	...
	elseif ( $page_type['shop'] && ! $page_type['product'] ) {
		$layout = $bicycleshop_a13->get_option( 'shop_content_layout' );

		//only on pages where list of products are displayed
		if ( is_shop() || is_product_taxonomy() ) {
			$mid_classes[] = 'shop-columns-'.$bicycleshop_a13->get_option( 'shop_products_columns' );
		}
	} //product
	elseif ( $page_type['product'] ) {
		$layout = $bicycleshop_a13->get_option( 'product_content_layout' );
	} //page
	elseif ($page ) {
		$layout_option = $bicycleshop_a13->bicycleshop_get_meta( '_content_layout', $meta_id );
		$layout        = $layout_option === 'global' ? $bicycleshop_a13->get_option( 'page_content_layout' ):
			$layout_option;

		//in content padding
		$top_bottom_padding = $bicycleshop_a13->bicycleshop_get_meta('_content_padding');
		...
		$side_padding = $bicycleshop_a13->bicycleshop_get_meta('_content_side_padding');
		if($side_padding === 'off'){
			$mid_classes[] = 'no-side-space';
		}
	} //single post
	elseif ( $post ) {
		$layout = $bicycleshop_a13->get_option( 'post_content_layout' );
	} //blog type
	elseif ( $page_type['blog_type'] ) {
		$layout = $bicycleshop_a13->get_option( 'blog_content_layout' );

		//in content padding
		$top_bottom_padding = $bicycleshop_a13->get_option( 'blog_content_padding' );
		if($top_bottom_padding === 'top'){
			$mid_classes[] = 'no-bottom-space';
		}
		elseif($top_bottom_padding === 'bottom'){
	...
	if ( 
		 $attachment ||
	     bicycleshop_has_active_sidebar() === false
	) {
		$force_full_width = true;
	}

	function bicycleshop__inner__set_full_width( &$mid_classes ) {
		define( 'BICYCLESHOP_NO_SIDEBARS', true ); /* so we don't have to check again in sidebar.php */
		$mid_classes[] = 'no-sidebars';
	}

	function bicycleshop__inner__set_sidebar_class( &$mid_classes, $sidebar ) {
		if ( ( $sidebar == 'off' ) ) {
			bicycleshop__inner__set_full_width( $mid_classes );
		} else {
	...
	function bicycleshop__inner__check_for_rtl_sidebar_version( $option_name ) {
		global $bicycleshop_a13;
		$sidebar = $bicycleshop_a13->get_option( $option_name );
		if ( is_rtl() ){
			$rtl_opposites = array(
			...
			$sidebar = $bicycleshop_a13->get_option( $option_name.'_rtl', $rtl_opposites[$sidebar] );
		}
	...
	elseif ( $shop && ! $product ) {
		bicycleshop__inner__set_sidebar_class( $mid_classes, bicycleshop__inner__check_for_rtl_sidebar_version( 'shop_sidebar' ) );
	} //product type
	elseif ( $product ) {
		bicycleshop__inner__set_sidebar_class( $mid_classes, bicycleshop__inner__check_for_rtl_sidebar_version( 'product_sidebar' ) );
	} //blog type
	elseif ( $page_type['blog_type'] ) {
		bicycleshop__inner__set_sidebar_class( $mid_classes, bicycleshop__inner__check_for_rtl_sidebar_version( 'blog_sidebar' ) );
	} //single post
	elseif ( $post ) {
		bicycleshop__inner__set_sidebar_class( $mid_classes, bicycleshop__inner__check_for_rtl_sidebar_version( 'post_sidebar' ) );
	} //single page
	elseif ( $page ) {
		...
		if( $sidebar === 'G' || $sidebar === '' ){
			$sidebar = bicycleshop__inner__check_for_rtl_sidebar_version( 'page_sidebar' );
		}
		...
		bicycleshop__inner__set_sidebar_class( $mid_classes, $sidebar );
	}
 ...
 */
function bicycleshop_check_for_renamed_templates(){
	...
	$checked_name = bicycleshop_proper_page_template_name($current_name);
 ...
 *
 */
function bicycleshop_faster_google_fonts($urls, $relation_type){
	if('preconnect' === $relation_type){
		global $bicycleshop_a13;

		$standard_fonts = array_keys( $bicycleshop_a13->bicycleshop_get_standard_fonts_list() );
		$options_fonts = array(
			$bicycleshop_a13->get_option( 'nav_menu_fonts' ),
			$bicycleshop_a13->get_option( 'titles_fonts' ),
			$bicycleshop_a13->get_option( 'normal_fonts' ),
			//default to titles fonts as it was in previous versions
			$bicycleshop_a13->get_option( 'logo_fonts', $bicycleshop_a13->get_option( 'titles_fonts' ) ),
		);
...
add_filter( 'wp_resource_hints', 'bicycleshop_faster_google_fonts', 10, 2 );
 ...
 *
 */
function bicycleshop_get_theme_web_fonts($as_array = false){
	global $bicycleshop_a13;
